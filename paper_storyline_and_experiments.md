...
### ⏳ 实验 10：自适应动态阈值 (Context-Aware Dynamic Threshold)
* **实验目的**：探索消灭人工调参 $\tau$，实现基于上下文的自适应投机采样机制。
* **实验设计**：
  1. 放弃全局固定的阈值 $\tau$。改为计算当前 Token 的相对熵突变（Relative Entropy Spike），例如：判断当前 $H(X)$ 是否显著高于过去 5 个 Token 的滑动平均熵。
  2. 测试加入“生成位置衰减”策略，即在生成刚开始（最难的部分）采取较严的动态阈值，在生成结尾（通常是套话或句号）自动放宽阈值。
  3. 对比“固定最佳 $\tau$”与“无参数动态 $\tau$”在多数据集上的鲁棒性。
* **预期结论**：动态阈值策略能够以零调参的代价（Parameter-free），在各种长度和难度的 Prompt 下达到甚至超越最佳手工阈值的加速效果，提升系统的智能化程度。
